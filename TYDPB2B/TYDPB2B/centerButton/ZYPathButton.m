...
- (void)configureViewsLayoutWithButtonSize:(CGSize)centerButtonSize {
    ...
    //
    self.folZYenter = CGPointMake(self.bloomSize.width / 2, self.bloomSize.height - 32.5f);
...
//    self.frame = CGRectMake(0, 0, self.foldedSize.width, self.foldedSize.height);
    self.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width/5>65*Width?[[UIScreen mainScreen] bounds].size.width/5:65.0*Width, 65.0*Width);
    debugLog(@"ffffframe:%@",NSStringFromCGRect(self.frame));
//    self.backgroundColor=[UIColor blackColor];
//    NSLog(@"framememe:%@",NSStringFromCGRect(self.frame));
    ...
    //
//    _pathCenterButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.centerImage.size.width+30, self.centerImage.size.height+30)];
    
//    _pathCenterButton一定是个正方形
    _pathCenterButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 55, 55)];
    
    
    [_pathCenterButton setBackgroundImage:self.centerImage forState:UIControlStateNormal];
    [_pathCenterButton setBackgroundImage:self.centerImage forState:UIControlStateHighlighted];
...
- (void)pathCenterButtonBloom {
    ...
    // ZYPathButton Delegate
    //
    [_pathCenterButton setBackgroundImage:self.centerHighlightedImage forState:UIControlStateNormal];
...
- (void)itemButtonTapped:(ZYPathItemButton *)itemButton {
    ...
    if ([_delegate respondsToSelector:@selector(pathButton:clickItemButtonAtIndex:)]) {
        [self.pathCenterButton setBackgroundImage:self.centerImage forState:UIControlStateNormal];
