...
- (UILabel *)numberLabel{
    if (!_numberLabel) {
        ...
        _numberLabel.text = [NSString stringWithFormat:@"%lu/%lu",self.index +1,self.imageArray.count];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    self.numberLabel.text = [NSString stringWithFormat:@"%lu/%lu",self.index+1,self.imageArray.count];
}
- (void)layoutSubviews{
    ...
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        STImageVIew * imageView = (STImageVIew *) obj;
        obj.frame = CGRectMake(SpaceWidth + (Screen_Width+20) * idx, ((ScreenHeight-NavHeight - imageView.image.size.height)/2-NavHeight),Screen_Width,Screen_Width*(imageView.image.size.height)/(imageView.image.size.width));
        CGPoint tmpPoint = CGPointMake(Screen_Width/2+SpaceWidth + (Screen_Width+20) * idx, Screen_Height/2);
        obj.center = tmpPoint;
    }];
}
- (void)show{
